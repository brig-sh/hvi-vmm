...
impl GatewayRelay {
    ...
    /// blocking on a peer nobody reads any more. macOS refuses the shutdown
    /// once the peer has half-closed, so a gateway that stops sending but keeps
    /// the socket open still accepts the device's writes there until its buffer
    /// fills.
    ///
    /// # Errors
    ///
...
pub struct VirtioNet {
    ...
    /// Whether the tap has already refused a frame, so the drop is reported
    /// once.
    tap_drop_reported: bool,
...
impl VirtioNet {
    ...
    fn process_tx(&mut self, mem: &GuestRam) {
        ...
        for _ in 0..pending {
            ...
            if let Some(frame) = self.read_tx_frame(mem, head) {
                ...
                // else too.
                // The ledger records the frame before the write, so a frame
                // the tap's full send buffer refuses still appears in it.
    ...
    fn read_tx_frame(&self, mem: &GuestRam, head: u16) -> Option<Vec<u8>> {
        ...
        for _ in 0..q.size() {
            ...
            if flags & crate::virtio::VIRTQ_DESC_F_WRITE == 0 {
    ...
    fn inject_rx(&mut self, mem: &GuestRam, frame: &[u8]) {
        ...
        for _ in 0..rx.size() {
            ...
            if flags & crate::virtio::VIRTQ_DESC_F_WRITE != 0 && written < out.len() {
        ...
        // to reject.
        //
        // The chain stays at the same avail slot, so a head that can never
        // hold a frame blocks the ones behind it until the guest reposts.
        // Linux and Unikraft post uniform buffers, so neither reaches that;
        // completing it with a used length of 0 is the escape hatch if some
        // driver ever does.
        if written < out.len() {
            return;
        }
        ...
        if let Some(tap) = self.tap.as_mut() {
            match tap.write_all(&crate::tap::prepend_vnet_hdr(frame)) {
                ...
                Err(e) if e.kind() == io::ErrorKind::WouldBlock => {
                    // The tap's send buffer is full. The frame is dropped
                    // rather than blocking the vCPU under the device lock;
                    // `tap::open` explains the flag.
                    if !self.tap_drop_reported {
                        self.tap_drop_reported = true;
                        eprintln!(
                ...
                Err(_) => {
                    // The tap is gone, so there is nothing to report and
                    // nothing to retry.
                }
            }
        }
...
mod tests {
    ...
    mod relay {
        ...
        // A gateway that stops sending but keeps the socket ends the relay too,
        // and the device's writes fail rather than queue for it. macOS refuses
        // the shutdown once the peer has half-closed, so the write goes through
        // there.
        #[cfg(target_os = "linux")]
        #[test]
    ...
    mod tap_relay {
        ...
        /// too small for a test's frames fails the send instead of blocking
        /// it, and its send buffer is raised above the host's default.
    ...
    // num_buffers is 1, so a frame that does not fit cannot continue into the
    // next buffer: drop it rather than complete a truncated packet.
    #[test]
