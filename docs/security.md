...
allow rule for `file-ioctl` on `/dev/tty`, plus one rule per virtio-fs export
by resolved root path: `file-read*` for a read-only share, `file-read*
file-write*` for a writable one.
